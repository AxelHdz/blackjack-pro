...
export function PlayingCard({ card, hidden = false, delay = 0, owner = "player" }: PlayingCardProps) {
  ...
  return (
    <div
      className={cn(
        "relative w-28 h-36 md:w-32 md:h-44 rounded-xl border-2 border-gray-300 bg-white shadow-xl",
      ...
      {hidden ? (
        ...
        <div className="absolute inset-0 flex flex-col">
          ...
          <div className="absolute top-1 left-1.5 md:top-1.5 md:left-2 flex flex-col items-center">
            <div className={cn("text-xl md:text-2xl font-bold leading-none", isRed ? "text-red-600" : "text-gray-900")}>
              {card.rank}
            </div>
            <div className={cn("text-xl md:text-2xl leading-none -mt-0.5", isRed ? "text-red-600" : "text-gray-900")}>
              {card.suit}
            </div>
          </div>
          ...
          {!isFaceCard && card.rank !== "A" && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className={cn("text-5xl md:text-6xl", isRed ? "text-red-600" : "text-gray-900")}>{card.suit}</div>
            </div>
          )}
          ...
          {card.rank === "A" && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className={cn("text-6xl md:text-7xl", isRed ? "text-red-600" : "text-gray-900")}>{card.suit}</div>
            </div>
          )}
          ...
          {isFaceCard && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex flex-col items-center gap-1">
                <div className={cn("text-5xl md:text-6xl font-bold", isRed ? "text-red-600" : "text-gray-900")}>
                  {card.rank}
                </div>
                <div className={cn("text-4xl md:text-5xl", isRed ? "text-red-600" : "text-gray-900")}>{card.suit}</div>
              </div>
            </div>
          )}
